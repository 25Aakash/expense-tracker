...
const AddIncomeScreen = ({ navigation, route }) => {
  ...
  const loadCategories = async () => {
    try {
      ...
      const incomeCategories = categoriesArray.map((name, index) => ({
        _id: `income_${index}_${name}`,
        name,
        type: 'income'
      }));
      ...
      if (incomeCategories.length > 0 && !form.category) {
        setForm(prev => ({ ...prev, category: incomeCategories[0].name }));
      }
    } catch (error) {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      await categoryAPI.addIncomeCategory(newCategoryName.trim());
      setNewCategoryName('');
      ...
      >
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <Surface style={styles.typeSwitchCard}>
            ...
            <View style={styles.typeSwitch}>
              <TouchableOpacity
                ...
                onPress={() => {
                  if (loading) return;
                  // Navigate to Add Expense screen with current form data
              ...
              >
                ...
                <Text style={styles.typeSwitchInactiveText}>Expense</Text>
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                ...
                <Text style={styles.typeSwitchActiveText}>Income</Text>
              </TouchableOpacity>
            </View>
          </Surface>
          {/* Amount Card */}
          ...
          <Surface style={styles.formCard}>
            <View style={styles.fieldHeader}>
              <View style={styles.fieldIconContainer}>
                <Icon name="note-text" size={18} color="#10b981" />
              </View>
